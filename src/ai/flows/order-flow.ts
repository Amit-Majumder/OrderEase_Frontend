...
/**
 * Marks an order as complete in the backend.
 * @param id - The id of the order to mark as complete.
 * @returns A promise that resolves to true if successful, false otherwise.
 */
export async function completeOrder(id: string): Promise<{ success: boolean }> {
    try {
        const res = await axios.patch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/kitchen/status/${id}`);
        return { success: res.data?.status === 'done' };
    } catch (err) {
        console.error(`Error completing order ${id}:`, err);
